...
void fat_file_free(fat_file file, fat_file parent){
 u32 last_cluster = 0, next_cluster = 0;

 //"elimino el primer cluster"
    last_cluster = set_first_cluster(file->dentry, FAT_CLUSTER_FREE);
    last_cluster = file->start_cluster;

//recorro todos los clusters (excepto el primero) y voy "eliminandolos"
    while (!fat_table_cluster_is_EOC(last_cluster)) {
        // If there was an error, we continue with the function.
        //Si hubo un error, continuamos con la función
        fat_table_set_next_cluster(file->table, last_cluster, FAT_CLUSTER_FREE);
        next_cluster = fat_table_get_next_cluster(file->table, last_cluster);
        last_cluster = next_cluster;
    }

    DEBUG("pasa por el bucle de fat file free");
    file->dentry->file_size = 0; // Overwrite with new sizeSobrescribir con nuevo tamaño
    //marco el directorio como vacio
    file->dentry->base_name[0] = FILE_FOR_DELETION;
    fill_dentry_time_now(file->dentry, false, true);
    //escribo los cambios en el disco
    write_dir_entry(parent, file);
